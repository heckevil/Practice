	...
	<c:forEach var="item" items="${requestScope.list }">
		<tr class="record" onclick="moveToDetail(${item.iboard});">
			...
			<c:choose>
				...
				<c:otherwise>
					<c:set var="img"
						value="/res/img/user/${item.iuser }/${itme.profileImg }" />
				</c:otherwise>
			</c:choose>
			<td>${item.writerNm }<img src="${img }" class="profileImg">
			</td>
			<td>${item.regdt }</td>
		</tr>
	</c:forEach>
...
<div>
	<c:forEach begin="1" end="${requestScope.pagingCnt }" var="page">
		...
		<c:choose>
			...
			<c:otherwise>
				<span><a
					href="boardList?cPage=${page}&searchType=${param.searchType}&searchText=${param.searchText}">${page}</a></span>
			</c:otherwise>
		</c:choose>
